...
from lib import config_utils, data_utils, visutils, utils
...
class Imputation:
    def __init__(
        ...
        self.ifDate = ifDate
        self.ifCond = ifCond
        self.visualize = visualize  # Visualization toggle
        ...
        if self.method == Method.STRIDE:
            ...
            print(self.config_file_train, self.config)
        ...
        scheduler = DPMSolverMultistepScheduler(num_train_timesteps=1000,
                                                prediction_type=self.config.training_settings.prediction_type)
    ...
    def impute_sample(
        ...
        #    self._visualize_inputs(batch, vis_prefix)
        ...
        #y_pred = impute_sequence_STRIDE(self.model, batch, self.temporal_window, self.noise_scheduler,self.num_inference_steps, ifDate=self.ifDate, ifCond=self.ifCond)
    ...
    def linear_2pct_stretch(arr, nodata=None):
        """
        Apply 2%-98% linear stretch to the input array and output 8-bit result.
    ...
    def _visualize_inputs(self, batch: Dict[str, Any], prefix: str):
        """Visualize input data."""
        if not self.visualize or not self.vis_dir:
            return

        # Create subdirectory
        input_dir = Path(self.vis_dir) / f"{prefix}_{self.vis_count}" / "inputs"
        input_dir.mkdir(parents=True, exist_ok=True)

        # Extract data
        x = batch['x'].cpu().numpy()  # Input image [B, T, C, H, W]
        mask = batch['masks'].cpu().numpy()  # Mask [B, T, 1, H, W]

        # Conditional data (if available)
        cond = None
        if 'cond' in batch and batch['cond'] is not None:
        ...
        date = None
        if 'position_days' in batch and batch['position_days'] is not None:
            date = batch['position_days'].cpu().numpy()

        B, T = x.shape[:2]
        rgb_idx = [0, 1, 2]  # Assume first 3 channels are RGB

        # Save batch info
        with open(input_dir / "batch_info.txt", "w") as f:
            f.write(f"Batch size: {B}\n")
            f.write(f"Sequence length: {T}\n")
            f.write(f"Channels: {x.shape[2]}\n")
            f.write(f"Spatial dimensions: {x.shape[3]} x {x.shape[4]}\n")
            if date is not None:
                f.write(f"Date range: {date.min()} to {date.max()}\n")

        # Visualize each sample and time step
        for b in range(min(B, 2)):  # Only visualize first 2 samples
            for t in range(min(T, 35)):  # Only visualize first 35 time steps
                # Visualize input x
                if x.shape[2] >= 3:  # Ensure enough channels
                    x_rgb = x[b, t, rgb_idx]  # [3, H, W]
                    for c in range(3):
                        x_rgb[c] = self.linear_2pct_stretch(x_rgb[c])
                    x_rgb = x_rgb.transpose(1, 2, 0)  # [H, W, 3]
                    x_rgb = np.clip(x_rgb, 0, 255).astype(np.uint8)
                    Image.fromarray(x_rgb).save(input_dir / f'x_b{b}_t{t}.png')

                # Visualize mask
                m = mask[b, t, 0]  # [H, W]
                m = (m * 255).astype(np.uint8)
                Image.fromarray(m, mode='L').save(input_dir / f'mask_b{b}_t{t}.png')

                # Visualize conditional data (if available)
                if cond is not None and cond.shape[2] >= 3:
                    cond_rgb = cond[b, t, rgb_idx]
                    for c in range(3):
                        cond_rgb[c] = self.linear_2pct_stretch(cond_rgb[c])
                    cond_rgb = cond_rgb.transpose(1, 2, 0)
                    cond_rgb = np.clip(cond_rgb, 0, 255).astype(np.uint8)
                    Image.fromarray(cond_rgb).save(input_dir / f'cond_b{b}_t{t}.png')

                # Visualize target data (if available, for comparison)
                if y is not None and y.shape[2] >= 3:
                    y_rgb = y[b, t, rgb_idx]
                    for c in range(3):
                        y_rgb[c] = self.linear_2pct_stretch(y_rgb[c])
                    y_rgb = y_rgb.transpose(1, 2, 0)
                    y_rgb = np.clip(y_rgb, 0, 255).astype(np.uint8)
                    Image.fromarray(y_rgb).save(input_dir / f'target_b{b}_t{t}.png')

                # Save time step info
                with open(input_dir / f"timestep_info_b{b}_t{t}.txt", "w") as f:
                    f.write(f"Sample {b}, Time step {t}\n")
                    if date is not None:
                        f.write(f"Date: {date[b, t]}\n")
                    if x.shape[2] >= 3:
                        f.write(f"Input x range: [{x[b, t].min():.4f}, {x[b, t].max():.4f}]\n")
                    if y is not None and y.shape[2] >= 3:
                        f.write(f"Target y range: [{y[b, t].min():.4f}, {y[b, t].max():.4f}]\n")
                    f.write(f"Mask coverage: {mask[b, t, 0].mean():.4f}\n")

        print(f"Input data visualization saved to: {input_dir}")
    ...
    def _visualize_outputs(self, batch: Dict[str, Any], y_pred: Tensor, prefix: str):
        ...
        # Create subdirectory
        output_dir = Path(self.vis_dir) / f"{prefix}_{self.vis_count}" / "outputs"
        output_dir.mkdir(parents=True, exist_ok=True)

        # Extract data
        x = batch['x'].cpu().numpy()  # Input image
        mask = batch['masks'].cpu().numpy()  # Mask
        y_pred_np = y_pred.cpu().numpy()  # Prediction results
        ...
        for b in range(min(B, 2)):  # Only visualize first 2 samples
            for t in range(min(T, 35)):  # Only visualize first 35 time steps
                ...
                if y_pred_np.shape[2] >= 3:
                    pred_rgb = y_pred_np[b, t, rgb_idx]
                    for c in range(3):
                ...
                # Create comparison: input vs prediction vs target (if available)
                if x.shape[2] >= 3 and y_pred_np.shape[2] >= 3:
                    # Horizontally concatenate input, prediction, and target
                    images_to_concat = []
                    ...
                    if y is not None and y.shape[2] >= 3:
                        ...
                        for c in range(3):
                            y_rgb[c] = self.linear_2pct_stretch(y_rgb[c])
                        y_rgb = y_rgb.transpose(1, 2, 0)
                        images_to_concat.append(np.clip(y_rgb, 0, 255).astype(np.uint8))

                    # Horizontal concatenation
                    comparison = np.concatenate(images_to_concat, axis=1)

                    # Add labels
                    comparison_with_labels = self._add_labels_to_comparison(comparison,
                                                                            ['Input', 'Prediction',
                                                                             'Target'] if y is not None else ['Input',
                                                                                                              'Prediction'])
                    Image.fromarray(comparison_with_labels).save(output_dir / f'comparison_b{b}_t{t}.png')

                # Save time step evaluation info
        ...
        print(f"Output visualization saved to: {output_dir}")
    ...
    def _generate_html_report(self, x, y, y_pred, mask, B, T, prefix):
        ...
        <html>
        <head>
            <title>STRIDE Reconstruction Summary - {prefix}</title>
            <style>
        ...
        <body>
            <div class="header">
                <h1>STRIDE Reconstruction Summary - {prefix}</h1>
                <p>Generation time: {time.strftime('%Y-%m-%d %H:%M:%S')}</p>
            </div>
...
def evaluate_FM(model, x_obs, mask, date, cond):
    ...
    Image-domain inference engine based on toy_eval logic.
    ...
    # Initialization
    # =====================
    x_0 = x_obs                     # Observed / GT (trusted in clear-sky regions)
...
def impute_sequence_STRIDE(
    ...
    #print("seq_length", seq_length, temporal_window,x.shape,cond.shape,mask.shape,torch.max(x),torch.min(x))
