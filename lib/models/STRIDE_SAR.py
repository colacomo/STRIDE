...
class DateEmbedder(nn.Module):
    """Date embedder for processing temporal information in time-series data.
    Provides date information (e.g., satellite capture time) for each time step
    using fixed-frequency sine/cosine encoding."""
...
class LabelEmbedder(nn.Module):
    """Class label embedder with dropout for classifier-free guidance.
    labels -> embedding_table -> vector.
    Supports classifier-free guidance (CFG): randomly drop labels during training,
    use conditional/unconditional interpolation during inference.
    token_drop implements label dropout."""
...
def upsample_interaction_matrix(matrix, orig_shape, target_shape, mode='nearest'):
    """
    Correctly upsample interaction/state matrix while preserving spatial position alignment.
    ...
    mode: interpolation mode. For state/reward matrices, 'nearest' is recommended
          for exact replication; use 'bilinear' (with align_corners=False) if
    ...
    # Restore Key's spatial structure: [B, N1, N2] -> [B, N1, h, w]
    matrix_spatial_k = matrix.view(B, N1, h, w)

    # Interpolate on Key's spatial dimensions: [B, N1, H, W]
    matrix_k_up = F.interpolate(matrix_spatial_k, size=(H, W), mode=mode)

    # Flatten Key dimension: [B, N1, H*W]
    ...
    # Restore Query's spatial structure: [B, H*W, N1] -> [B, H*W, h, w]
    matrix_spatial_q = matrix_k_up_t.view(B, H * W, h, w)

    # Interpolate on Query's spatial dimensions: [B, H*W, H, W]
    matrix_q_up = F.interpolate(matrix_spatial_q, size=(H, W), mode=mode)

    # Flatten Query dimension: [B, H*W, H*W]
...
class AdaLNSelfAttention(nn.Module):
    ...
    def forward(self, x, date_emb, attn_bias=None, attn_mask=None):
        ...
        self.first_attn = attn.detach()
        if attn_bias is not None:
            # Use optical info from nearby timestamps
        ...
        if attn_mask is not None and attn_mask.shape[1]<=256:
            if attn_mask.shape[1]<=35:
                ...
                # Reshape 1D sequence back to 16x16 spatial grid
                mask_2d = attn_mask.view(-1, 1, H, W)
                # Use max pooling. If any pixel in a 2x2 region is cloud, treat as cloud after downsampling to avoid missed detections.
                p_size = int(H/8)
...
class ConvMLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        # groups=hidden_features ensures extremely low computation
...
class AdaLNCrossAttention(nn.Module):
    ...
    def forward(self, x, sar, date_emb, attn_bias=None, attn_mask=None):
        # x, sar: (B*, N, D)
        #print("cross")
        shift, scale, gate = self.adaLN(date_emb).chunk(3, dim=-1)
        ...
        k = k.reshape(B, N, self.num_heads, D // self.num_heads).permute(0, 2, 1, 3)
        v = v.reshape(B, N, self.num_heads, D // self.num_heads).permute(0, 2, 1, 3)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        self.first_attn = attn.detach()
        if attn_bias is not None:
            # Use SAR info from nearby timestamps
            attn = attn + attn_bias.unsqueeze(1)
        self.sec_attn = attn.detach()
        cloud_reward = None
        ...
        out = (attn @ v).transpose(1, 2).reshape(B, N, D)
        return gate.unsqueeze(1) * self.proj(out), attn


class STBlock(nn.Module):
    def __init__(self, dim, num_heads, cross=True):
        super().__init__()
        self.cross = cross
        # Temporal
        ...
        if cross == True:
            self.temporal_cross = AdaLNCrossAttention(dim, num_heads)
            self.spatial_cross = AdaLNCrossAttention(dim, num_heads)
    ...
    def forward(self, x, sar, t_emb, s_emb, time_bias_self, totalmask_t, totalmask_s, cloudmask_t, cloudmask_s, B, T):
        ...
        x_t = x_t + tem_out
        if self.cross==True:
            cross_tem_out, _ = self.temporal_cross(x_t, sar_t, t_emb)#, tbc)  # , cloudmask_t)
            x_t = x_t + cross_tem_out
        ...
        x = x + spa_out
        if self.cross==True:
            cross_spa_out, _ = self.spatial_cross(x, sar, s_emb)  # , attn_mask=cloudmask_s)
            x = x + cross_spa_out
...
class MMTE(nn.Module):
    ...
    def __init__(self, hidden_dim, num_freqs=1, init_alpha=0.01, max_period=365.0):
        ...
        # Allow different frequencies to have different contributions
        ...
        # Initialized to 0, i.e., sigmoid(0) = 0.5, representing equal contributions initially
    ...
    def forward(self, dates):
        """
        dates: (B, T)
        return: bias matrix of shape (B, T, T)
        """
        B, T = dates.shape
        ...
        # Compute phase: 2 * pi * f * delta_t / T_max
        angles = 2 * math.pi * m * diff.unsqueeze(-1) / self.max_period

        # Weighted sum of cosine values at different frequencies
        ...
        # --- Dynamic balance mixing ---
        # beta = 0: pure linear translation; beta = 1: pure periodicity
...
class ASTT_Block(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, num_frames=16, cross=False, **kwargs):
        super().__init__()
    ...
    def forward(self, x, c_t, c_s, dates, SAR, cloud_mask=None, total_mask=None, return_attn=False):
        ...
        total_mask = F.interpolate(
            total_mask.float(), size=(1, H, W), mode='nearest')
...
class UpSample(nn.Module):
    ...
    def forward(self, x, cond, H, W):
        # x: [B*T, C_in, H, W] (after flattening)
        # skip: [B*T, C_skip, H, W] (after flattening)
        # prompt_emb: [B*T, P_dim] (after flattening)
...
class CMTS_Embedder(nn.Module):
    """Decompose cloud mask into temporal and spatial vector embeddings.

    Temporal vector: [B*H*W, hidden_size] - time-series patterns at each spatial location
    Spatial vector: [B*T, hidden_size] - spatial distribution patterns at each timestep
    """
    ...
    def __init__(self, input_size, num_frames, hidden_size, patch_size=4):
        ...
        self.temporal_conv = nn.Sequential(
            ...
            # Process time series at each spatial location
    ...
    def forward(self, cm):
        ...
        # Temporal embedding: extract temporal features for each spatial location
...
class FinalLayer(nn.Module):
    """SDT final output layer.
    Maps Transformer output back to image patches. Uses AdaLN to inject conditions.
    Output dimension is patch_size^2 * out_channels."""
...
class STRIDE_SAR(nn.Module):
    """
    Sequential Denoising Transformer based on temporal Lie group.
    For satellite image time-series reconstruction.
    """
    ...
    def __init__(self, input_size=32, patch_size=2, in_channels=4, hidden_size=256, depth=3,
                 ...
                 learn_sigma=False, num_frames=10, cond_in_channels=3, cross_attention=True):
        super().__init__()
    ...
    def initialize_weights(self):
        ...
        #nn.init.constant_(self.deg_estimator.detector[3].bias, -1)
        ...
        # Zero-initialize adaLN modulation layers in SDT blocks
    ...
    def forward(
        ...
        """
        SDT forward pass.
        x: (B, T, C, H, W) input tensor (image or latent representation of image)
        ...
        # print("x",x.shape)
        # Use embedded features to detect anomalies
        # latent_mask: [B, T, 1, H_token, W_token]
        ...
        # Reconstruction loss does not backprop through degradation head; alpha only controls
        # the influence of predicted masks on attention.
        ...
        # SAR condition embedding
        if cond is not None:
            cond = cond.contiguous().view(-1, self.cond_in_channels, H, W)
            cond = self.cond_embedder(cond) + self.pos_embed
...
def STRIDE_SAR_10():
    return STRIDE_SAR(depth=4, in_channels=10, hidden_size=256, patch_size=4, num_heads=4,
...
if __name__ == '__main__':
    ...
    net = STRIDE_SAR_10().to("cuda" if torch.cuda.is_available() else "cpu")
    b, s, size = 1, 10, 128
