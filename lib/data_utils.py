...
def get_dataloader_simple_sampling(
        config: DictConfig,
        phase: str,
    ...
    """Simplified version with just ratio-based sampling."""
    ...
    if sample_ratio < 1.0:
        ...
        if logger:
            logger.info(f"Sampled {sample_size}/{total_size} samples ({sample_ratio:.0%})")

    # Check for variable sequence length
    variable_seq_length = (
            hasattr(dset, 'variable_seq_length') and
            getattr(dset, 'variable_seq_length', False) and
            config.training_settings.batch_size > 1
    )

    # Determine shuffle - don't shuffle test data
    shuffle = phase.lower() != 'test' and config.misc.run_mode != 'test'

    # Set up collate function if needed
    collate_fn = None
    if variable_seq_length:
        collate_fn = partial(
            pad_collate,
            pad_value=config.method.get('pad_value', 0)
        )
    ...
    loader = torch.utils.data.DataLoader(
        ...
        drop_last=drop_last,
        persistent_workers=config.misc.num_workers > 0
    )
...
def get_dataset(config: DictConfig, phase: str, logger: Optional[logging.Logger] = None):
    ...
    assert config['misc']['run_mode'] in ['train', 'val', 'test', 'test_n', 'test_s']
    assert phase in ['train', 'val', 'train+val', 'test', 'test_n', 'test_s']
    ...
    if hasattr(config.data, 'date_seq') and isinstance(config.data.date_seq, str):
        try:
            import json
            config.data.date_seq = json.loads(config.data.date_seq)
        except:
            logger.warning("Failed to parse date_seq as JSON, trying eval")
            import ast
            config.data.date_seq = ast.literal_eval(config.data.date_seq)
    ...
    if 'hdf5_file' in config.data and isinstance(config.data.hdf5_file, DictConfig):
        ...
        phase_key = phase if phase in config.data.hdf5_file else 'test'
        dset = Dataset(
            hdf5_file=config.data.hdf5_file[phase_key],
            **without_keys(config.data, ['dataset', 'hdf5_file', 'split']),
            mask_kwargs=config.mask, augment=augment, split=phase
        )
    else:
        dset = Dataset(
            **without_keys(config.data, ['dataset', 'preprocessed', 'render_occluded_above_p', 'return_cloud_mask', 'split']),
            mask_kwargs=config.mask, augment=augment, split=phase)
...
def compute_false_color(x: Tensor, index_rgb, index_nir):
    """
