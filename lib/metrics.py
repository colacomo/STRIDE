...
class EvalMetrics:
    ...
    def __call__(self, batch: Dict[str, Any], predicted: Tensor) -> Dict[str, float]:
        ...
        #print(target.shape, masks.shape, mask_valid, cloud_mask.shape)
        target, masks, mask_valid, cloud_mask = batch['y'], batch['masks'], None, batch['cloud_mask']
        #print("mec", torch.max(batch['y']),torch.min(batch['y']), torch.max(predicted),torch.min(predicted),
        #      torch.mean(batch['x'] - predicted), torch.mean(batch['y']-predicted) , predicted.shape)
        ...
        n_frames = B * T
        #print("metrics", n_frames,predicted.shape,target.shape,masks.shape,cloud_mask.shape)
        ...
        #cloud_mask = cloud_mask.view(n_frames, 1, H, W).expand(target.shape)
        #print("masks", torch.max(masks), torch.min(masks))
        ...
        # print(self.masked_metrics)
        if self.masked_metrics:
            #print(cloud_mask.shape,n_frames,H,W)
            #print("masks", torch.max(masks), torch.min(masks))
            ...
            #flag = cloud_mask.permute(2, 3, 0, 1).reshape(H * W, n_frames * C) == 0.
            #flag = cloud_mask.permute(2, 3, 0, 1).reshape(H * W, n_frames) == 0.
            flag = cloud_mask.permute(0, 2, 3, 1).reshape(n_frames * H * W) == 0.#0.#1.

            # Tensor shapes: (n_frames * H * W, C)
            #print(predicted.shape, target.shape,flag.shape)
            #predicted = predicted.permute(2, 3, 0, 1).reshape(H * W, n_frames * C)[flag]
            #target = target.permute(2, 3, 0, 1).reshape(H * W, n_frames * C)[flag]
            #masks = masks.permute(2, 3, 0, 1).reshape(H * W, n_frames * C)[flag]
        ...
        #print(predicted.shape,target.shape,torch.mean(predicted-target),torch.max(predicted),torch.max(target))
        if self.args.get('mae', False):
            metrics[f'{self.prefix}mae'] = self.mae(predicted, target)
            #print('mae', self.mae(predicted, target), 'rmse', self.rmse(predicted, target), 'mse', self.mse(predicted, target), 'psnr', self.psnr(predicted, target))

            if self.eval_occluded_observed:
                #print("masks",torch.max(masks),torch.min(masks), f'{self.prefix}mae')
                ...
                #print('mae',metrics[f'{self.prefix}mae'], metrics[f'{self.prefix}mae_occluded_input_pixels'],metrics[f'{self.prefix}mae_observed_input_pixels'])
        ...
        if self.args.get('rmse', False):
            ...
            if self.eval_occluded_observed:
                ...
                #print('rmse',metrics[f'{self.prefix}rmse'], metrics[f'{self.prefix}rmse_occluded_input_pixels'],metrics[f'{self.prefix}rmse_observed_input_pixels'])
        ...
        if self.args.get('mse', False):
            ...
            if self.eval_occluded_observed:
                ...
                #print('mse', f'{self.prefix}mse', metrics[f'{self.prefix}mse'], metrics[f'{self.prefix}mse_occluded_input_pixels'],metrics[f'{self.prefix}mse_observed_input_pixels'])
        ...
        if self.args.get('psnr', False):
            ...
            if self.eval_occluded_observed:
                ...
                print('psnr', metrics[f'{self.prefix}psnr'], metrics[f'{self.prefix}psnr_occluded_input_pixels'],metrics[f'{self.prefix}psnr_observed_input_pixels'])
        ...
        if self.args.get('sam', False):
            ...
            if self.eval_occluded_observed:
                if cloud_mask is None:
                    predicted = predicted.permute(0, 2, 3, 1).reshape(n_frames * H * W, C)
                    target = target.permute(0, 2, 3, 1).reshape(n_frames * H * W, C)
                    masks = masks.permute(0, 2, 3, 1).reshape(n_frames * H * W, C)
